...
Portiert aus `swiss-democracy-mcp`, wo derselbe Fall zuerst auffiel. Hier
lag er genauso: Am 18.9.2026 nannte `CLAUDE.md` `ruff==0.16.3`, waehrend
ein Dependabot-Lauf das `dev`-Extra in `pyproject.toml` laengst angehoben
hatte. Der Absatz war mit «an einer Stelle» ueberschrieben und wiederholte
den Wert daneben selbst; der Satz darunter verlangt, die Gates mit der
gepinnten Version zu fahren. Wer der Doku folgte, installierte das
abweichende ruff und erzeugte genau die Abweichungen, vor denen sie warnt.

Aufgefallen ist es nicht beim Lesen, sondern weil `check_ruff_pin.py` nach
einem Neuaufsetzen des Arbeitszweigs eine andere Version meldete als beim
Lauf davor.

Gemessen wurde, dass kein bestehendes Gate das sieht: Mit der Kopie in der
Doku meldete `scripts/check_ruff_pin.py` «Ruff-Pin OK» — es vergleicht das
*installierte* ruff gegen `pyproject.toml` und liest keine Doku — und
`scripts/check_version_sync.py` prueft die Paketversion, nicht diese. Beide
blieben gruen. Der Fall war ungedeckt, nicht bloss theoretisch.
...
Die Kommentare an `_IN_DOKU` und `_markdown()` halten fest, woran fruehere
Fassungen gescheitert sind. Sie stehen hier unveraendert: Es sind Befunde,
keine Stilfragen, und sie gelten fuer dieses Repo genauso.
"""
