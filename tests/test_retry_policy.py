...
def test_retry_after_reads_both_rfc9110_forms() -> None:
    ...
    assert server.parse_retry_after(resp(429, {"Retry-After": "120"})) == 120.0

    later = format_datetime(datetime.now(UTC) + timedelta(seconds=90))
    seconds = server.parse_retry_after(resp(503, {"Retry-After": later}))
    ...
    assert server.parse_retry_after(resp(503, {"Retry-After": past})) == 0.0

    # Unparseable falls back to the curve. It must not crash on the error path,
    # which is the one path already going badly.
    assert server.parse_retry_after(resp(429, {"Retry-After": "bald"})) is None
    assert server.parse_retry_after(resp(429, {})) is None

    # 500 does not carry a meaningful Retry-After.
    assert server.parse_retry_after(resp(500, {"Retry-After": "120"})) is None
    assert server.parse_retry_after(None) is None


def test_backoff_is_jittered() -> None:
    delays = {server.compute_delay(3, None) for _ in range(300)}
...
def test_cap_binds_after_the_jitter() -> None:
    ...
    deep = {server.compute_delay(9, None) for _ in range(200)}
    assert max(deep) <= server.settings.retry_max_delay

    hinted = _retry_after_error("600")
    assert {server.compute_delay(1, hinted) for _ in range(100)} == {
        server.settings.retry_max_delay
    }


def test_retry_after_jitter_is_one_sided() -> None:
    """The source said when. Later is polite; earlier ignores the value read."""
    delays = {server.compute_delay(1, _retry_after_error("4")) for _ in range(300)}
