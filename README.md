...
- **Terms of service:** Data is subject to the ToS of each source — [SIK-ISEA](https://www.sik-isea.ch), [opendata.swiss](https://opendata.swiss/terms-of-use), [Nationalbibliothek](https://www.nb.admin.ch/). The opendata.swiss datasets carry open licences (CC0 / CC BY). For the National Library this project has **not** verified the terms of either endpoint. Where a record carries its own rights statement it is surfaced as `rights` (MARC 506/540) — many records carry none. The licence line in the attribution footer is the source's general statement, not a per-record one, and JSON responses carry no footer at all. Check with the source before redistributing.
...
The SDK supplies none of these on its own — `Server.server_info` says so
...
[`tests/test_server_identity.py`](tests/test_server_identity.py), across both
...
path would be a second truth next to `pyproject.toml` — the very drift
